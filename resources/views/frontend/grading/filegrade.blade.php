    ...
    <script>
         $(document).ready(function() {
            ...
            $('#imageForm').on('submit', function(event) {
                ...
                $.ajax({
                    url: "{{ route('yolov5-request') }}", // Route to your controller method
                    type: "POST",
                    data: formData,
                    ...
                    success: function(response) {
                        ...
                        detects.forEach(function(detection) {
                            $('#detections').append('<li>Class: ' + detection.name + ', Confidence: ' + detection.confidence + '</li>');
                        });

                        $('#annotatedImage').attr('src', 'data:image/png;base64,' + response.image).show();
                    },
